...
describe("addCustomInstructions", () => {
	test("should not include language preference in custom instructions", async () => {
		...
		expect(result).not.toContain("You should always speak and think in the French language")
	})

	test("works without language preference", async () => {
		const result = await addCustomInstructions(
			"mode instructions",
			"global instructions",
			"/test/path",
